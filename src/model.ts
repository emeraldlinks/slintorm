...
export async function createModelFactory(adapter: DBAdapter) {
  ...
  return function defineModel<T extends Record<string, any>>(
    ...
    const name =
      modelName || Object.keys(schemas).find((k) => schemas[k].table === tableName) || tableName;
    ...
    return {
      async insert(item: T) {
        ...
        if (adapter.driver === "mongodb") {
         ...
         const cols = Object.keys(item).filter(c => typeof item[c as keyof T] !== "object"); // ignore relations
let sql: string;
...
/////---->
      ...
      async update(filter: Partial<T>, partial: Partial<T>) {
  await ensure();
  ...
  if (adapter.driver === "mongodb") {
    ...
    const values: any[] = setCols.map((c) => partial[c as keyof T] ?? null); // ensure no undefined
    const sql = `UPDATE ${tableName} SET ${setClause} WHERE ${whereClause}`;
    await adapter.exec(sql, [...values, ...whereParams!]);
  }

  return { ...filter, ...partial } as T;
}
,
      ...
      async exists(filter: Partial<T>) {
        await ensure();
        if (!Object.keys(filter).length) throw new Error("Exists filter cannot be empty");
