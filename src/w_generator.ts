...
export default async function generateSchema(srcGlob = "src/**/*.ts") {
  ...
  let relComment = "";
  if (jsDocs.length) relComment = jsDocs.map(j => j.getComment() || "").join(" ");
  if (!relComment) {
    const trailingMatch = prop.getText().match(/\/\/\s*@relation\s+(.+)/);
    if (trailingMatch) relComment = trailingMatch[1];
  }
  if (!relComment) {
    ...
    if (leadingMatch) relComment = leadingMatch[1].trim();
  }
  ...
  if (relComment) {
    try {
      ...
      const rel: RelationDef = {
        sourceModel: name,
        fieldName: propName,
        kind: kindRaw as RelationDef["kind"],
        targetModel,
      };
      for (const p of parts.slice(1)) {
        const [k, v] = p.split(":").map(x => x.trim());
        if (k === "foreignKey") rel.foreignKey = v;
        if (k === "through") rel.through = v;
      }
  ...
  const output: Record<string, { fields: Record<string, string>; relations: RelationDef[]; table?: string }> = {};
