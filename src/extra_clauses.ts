import { QueryBuilder, Dialects as Dialect } from "./queryBuilder.js";
...
export class AdvancedQueryBuilder<T extends Record<string, any>> extends QueryBuilder<T> {
    ...
    protected buildSql() {
        ...
        if (this._groupBy.length) sql += " GROUP BY " + this._groupBy.map(c => `"${c}"`).join(", ");
     ...
     */
    static union<T extends Record<string, any>>(queries: AdvancedQueryBuilder<T>[], all: boolean = false) {
